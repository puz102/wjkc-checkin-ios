/*
  Quantumult X: capture login credentials from the login response and the
...
function save(token, cookie, calibrated) {
  ...
  var previousCookie = $prefs.valueForKey(cookieKey + hostMatch);
  $prefs.setValueForKey(token, baseKey + hostMatch);
        $prefs.setValueForKey(cookie, cookieKey + hostMatch);
        $prefs.setValueForKey(hostMatch, lastActiveKey);
  if (!calibrated || previousCookie !== cookie) {
    $notify(calibrated ? "网际快车登录凭证校准成功" : "网际快车登录凭证抓取成功", hostMatch,
